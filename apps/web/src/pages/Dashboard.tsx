...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-3xl glass-card bg-gradient-to-r from-slate-900 via-slate-900/90 to-emerald-950/40 border border-slate-800">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Here is your financial snapshot for July 2026. Your health score is{' '}
            <span className="text-emerald-400 font-bold">{healthScore.score}/1000</span> ({healthScore.grade}).
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="flex items-center gap-2 py-2.5 px-4 rounded-xl bg-slate-900 border border-slate-700 text-slate-200 text-xs font-semibold hover:border-emerald-500/40 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 py-2.5 px-4 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-bold text-xs shadow-lg shadow-emerald-500/20 hover:opacity-95 transition-all cursor-pointer"
          >
        ...
        >
          ...
          <div className="w-full grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-800/80 text-xs">
            <div className="p-2 rounded-xl bg-slate-900/60 border border-slate-800 text-center">
              ...
              <span className="font-bold text-emerald-400">{healthScore.savingsRatioScore}/300</span>
            </div>
            <div className="p-2 rounded-xl bg-slate-900/60 border border-slate-800 text-center">
              <span className="text-[10px] text-slate-400 block">Debt Safety</span>
              <span className="font-bold text-emerald-400">{healthScore.debtToIncomeScore}/250</span>
            </div>
          </div>
        </motion.div>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="p-5 rounded-3xl glass-card relative overflow-hidden bg-gradient-to-br from-slate-900 to-slate-950">
            ...
            <div className="mt-2 text-[11px] text-slate-400 flex items-center justify-between">
              <span>Assets: <strong className="text-emerald-400">{formatCurrency(totalAssets, true)}</strong></span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 rounded-3xl glass-card space-y-4">
          ...
          <div className="space-y-3">
            {healthScore.insights.map((insight, idx) => (
              <div
                key={idx}
                className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800 flex items-start gap-3 hover:border-emerald-500/30 transition-all"
              >
        ...
        <div className="p-6 rounded-3xl glass-card flex flex-col justify-between">
          ...
          <div className="space-y-3">
            ...
            <div>
              <div className="flex justify-between text-xs font-semibold mb-1">
                ...
                <span className="text-emerald-400">{formatCurrency(budgetSummary.needs.amount)}</span>
              </div>
              <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${budgetSummary.needs.percentage}%` }} />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-xs font-semibold mb-1">
                ...
                <span className="text-blue-400">{formatCurrency(budgetSummary.wants.amount)}</span>
              </div>
              <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${budgetSummary.wants.percentage}%` }} />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between text-xs font-semibold mb-1">
                ...
                <span className="text-teal-400">{formatCurrency(budgetSummary.savings.amount)}</span>
              </div>
              <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                <div className="h-full bg-teal-500 rounded-full" style={{ width: `${budgetSummary.savings.percentage}%` }} />
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full mt-4 py-2 px-3 rounded-xl bg-slate-900 border border-slate-700 hover:border-emerald-500/30 text-xs font-bold text-slate-200 transition-all text-center cursor-pointer"
          >
            ...
            {expenses.slice(0, 4).map((exp) => (
              <div key={exp.id} className="flex items-center justify-between p-3 rounded-2xl bg-slate-900/50 border border-slate-800/80">
                ...
                <div className="text-right">
                  ...
                  {exp.isRecurring && (
                    <span className="text-[9px] text-emerald-400 font-semibold uppercase">Recurring</span>
                  )}
                </div>
              </div>
            ))}
