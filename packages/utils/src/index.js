 ...
 */
export function calculateIndianTax(monthlyIncome, deductions80C = 150000, deductions80D = 25000, hraExemption = 0, otherDeductions = 0) {
 ...
 */
export function calculate50_30_20(monthlyIncome, cityTier = 'TIER_2') {
    let needsPct = 50;
    let wantsPct = 30;
    let savingsPct = 20;
    if (cityTier === 'TIER_1') {
 ...
 */
export function calculateSIP(monthlyInvestment, expectedReturnRatePct, timePeriodYears, annualStepUpPct = 0) {
 ...
 */
export function calculateEMI(principalAmount, annualInterestRatePct, tenureYears) {
 ...
 */
export function calculateFD(principal, annualRatePct, tenureYears) {
    const compoundingFrequency = 4; // Quarterly in India
    const maturityAmount = principal * Math.pow(1 + annualRatePct / (100 * compoundingFrequency), compoundingFrequency * tenureYears);
 ...
 */
export function calculateFinancialHealthScore(monthlyIncome, monthlySavings, monthlyDebtEmi, emergencyFundMonths, hasInsurance) {
    if (monthlyIncome <= 0) {
        return {
            score: 500,
            grade: 'FAIR',
    ...
    const insights = [];
    if (savingsRatio < 20)
        insights.push('Increase savings rate to at least 20% of your net monthly income.');
    if (dtiRatio > 35)
        insights.push('High DTI ratio (>35%). Consider debt consolidation or loan prepayment.');
    if (emergencyFundMonths < 6)
        insights.push(`Emergency fund covers ${emergencyFundMonths} months. Aim for at least 6 months.`);
    if (!hasInsurance)
        insights.push('Get a comprehensive term life and health insurance policy to protect your family.');
    if (totalScore >= 800)
        insights.push('Outstanding financial discipline! You are in the top 5% of earners in your bracket.');
    return {
        score: totalScore,
        grade,
